...
/* recv log from RS485 device */
void Serial1Read(uint8_t* res)
{
  uint8_t index=0;
  while (Serial1.available() > 0) {  // 如果Serial1有可用数据        
    uint8_t temp = Serial1.read();  // 读取数据
    // if(temp!='\r' or temp!='\n' or temp!='\r\n')
    // {
      res[index++] = temp;
    // }
  ...
  res[index]='\0';
  Serial.printf("Serial1Read:%s:Serial1Read\r\n",res);   // 打印到Arduino内部串口，你也可以直接使用Serial1.print(c)打印到Serial1
}
...
{    
    ...
    for (int i = 0; i < data->BufferSize; i++) {
        process_serial_input(data->Buffer[i]);
    }
