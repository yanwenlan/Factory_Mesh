...
/* TrafficLight status */
uint8_t red_on[8] = {0x01, 0x05, 0x00, 0x00, 0xFF, 0x00, 0x8C, 0x3A};




/* send command to RS485 device */
void sendToRS485(uint8_t * temp)
{
  Serial1.write(temp,8);
}
void sendToRS485_2(uint8_t * temp, uint8_t size)
{
  // uint8_t * at_cmd = hex2ascii(temp, size);
  Serial1.write(temp, size);
  uint8_t back[64];
  Serial1.read();
  printf("sendToRS485_2 in...");
  printf("%s", temp);
}
...
uint8_t * hex2ascii(uint8_t * temp, uint8_t size)
{
  uint8_t at_cmd[64];
  uint8_t index=0;
  for(uint8_t i=0;i<size;i++)
  {
     at_cmd[i] = (uint8_t)temp[i];index++;
  }
  at_cmd[index] = '\0';
  return at_cmd;
}
void recvCallback(SERVICE_LORA_RECEIVE_T * data)
{
    ...
    sendToRS485_2(data->Buffer, data->BufferSize);
    if (data->BufferSize == 2) {
        ...
        // }
        // red
        if(data->Buffer[0] == 0x01 && data->Buffer[1] == 0x00)
        {
          Serial.println("red on");sendToRS485(red_on);
        }     
        // Serial.print("\r\n");
    }
}
